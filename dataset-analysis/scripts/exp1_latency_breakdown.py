...
def get_warm_cold_diff(df, aggregates) -> pd.DataFrame:
    ...
    shift = 0
    default_limit = int(df_length / 2)
    i = 0
    # IMPORTANT: Skipping rows requires extending the range to compensate for sparse data
    # `shift` represents individual rows but diffs are always computed across two rows.
    # Therefore, we need to divide the range extension by two.
    while i < default_limit + int(shift / 2):
        ...
        if df['coldstart'][warm_index]:
            shift += 1
            logging.warning(f"No data for warm invocations of {warm_row}. Skipping index {warm_index}.")
            i += 1
            continue
        # Handle cold invocation data missing
        if not df['coldstart'][cold_index]:
            shift += 1
            logging.warning(f"No data for cold invocations of {row}. Skipping index {cold_index}.")
            i += 1
            continue
        ...
        assert df['coldstart'][cold_index] != df['coldstart'][warm_index], (
    ...
    # Summarize skipped missing data
    logging.info(f"Skipped {shift} invocations due to missing data.")
...
def plot_latency_breakdown_warm(df, agg):
    ...
    p = (
        ...
        + geom_text(aes(label=f"{agg}_latency_share_label"), position=position_fill(vjust=0.5),
                    size=11, color='black', fontweight='normal')  # format_string='{:.2%}'
        + labs(x='Application', y="Latency breakdown", fill='Activity')
        + theme_light(base_size=13)
        + theme(figure_size=(7, 2.5), legend_position=(1.05, 0.5),
               legend_background=element_rect(alpha=0), legend_title_align='left',
        ...
        + scale_fill_manual(['#fdb462','#80b1d3','#d9ffcf','#bebada','#ffffb3','#8dd3c7',
                            '#fccde5','#b3de69'])
        + guides(fill=guide_legend(nrow=6, title='Activity',
                                   title_position='top', byrow=True))
    )
...
def plot_latency_breakdown_warm_cold_diff(df_diff, agg):
    ...
    p = (
        ...
        + ylim(0, 1)
        + labs(x='Application', y=f"Latency-penalty\nbreakdown", fill='Activity')
        + theme_light(base_size=13)
        + theme(figure_size=(7, 2.5), legend_position=(1.05, 0.4),
               legend_background=element_rect(alpha=0), legend_title_align='left',
               legend_margin=0, legend_box_margin=0)
        # + scale_fill_brewer('qual', 'Set3', direction=-1)
        + scale_fill_manual(['#fdb462','#80b1d3','#d9ffcf','#bebada','#ffffb3','#8dd3c7',
                            '#fccde5','#b3de69'])
        + guides(fill=guide_legend(nrow=8, title='Activity',
                                   title_position='top', byrow=True))
        )
...
def plot_latency_breakdown_warm_slow_diff(df_slow):
    ...
    p = (
        ...
        # NOTE: automatically removes rows with negative values
        + ylim(0, 1)
        + labs(x='Application', y="Latency-penalty\nbreakdown", fill='Activity')
        + theme_light(base_size=12)
        + theme(figure_size=(7, 2.5), legend_position=(1.05, 0.53),
               legend_background=element_rect(alpha=0), legend_title_align='left',
               legend_margin=0, legend_box_margin=0)
        # + scale_fill_brewer('qual', 'Set3', direction=-1)
        + scale_fill_manual(['#fdb462','#80b1d3','#d9ffcf','#bebada','#ffffb3','#8dd3c7',
