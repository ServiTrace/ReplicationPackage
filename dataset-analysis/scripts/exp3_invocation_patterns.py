...
df_filtered['app_symbol'] = (df_filtered['app'].str
                      .replace('thumbnail_generator', 'App-B')
                      .replace('event_processing', 'App-C')
                      .replace('image_processing', 'App-D')
                      .replace('realworld_backend', 'App-F'))
df_filtered['app_symbol'] = pd.Categorical(df_filtered['app_symbol'], categories=['App-B','App-C','App-D', 'App-F'], ordered=True)
...
p = (
    ggplot(df_filtered)
    + aes(x='duration_sec', color='iteration', linetype='iteration')
    + labs(
        x='Trace duration [s]', y=f"ECDF", color='Workload type', linetype='Workload type')
    + facet_wrap('app_symbol', scales = 'free_x', nrow=1)
    + theme_light(base_size=16)
    + theme(figure_size=(12, 2.5), legend_position='right',
